...
import { MapPin, Calendar, Users, Sprout, Lightbulb, Heart } from "lucide-react";
...
export default function Events() {
  ...
  const getButtonColor = (eventType: string) => {
    switch(eventType) {
      case 'cleanup': return 'bg-forest hover:bg-green-800';
      case 'workshop': return 'bg-university hover:bg-blue-800';
      case 'competition': return 'bg-warning text-text-dark hover:bg-yellow-500';
      default: return 'bg-forest hover:bg-green-800';
    }
  };
      ...
      <section className="py-16 bg-bg-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-text-dark mb-4">Events & Initiatives</h2>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Join our waste management activities and university society initiatives for a cleaner campus
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {events && events.length > 0 ? (
            events.map((event: Event) => (
              <Card key={event.id} className="bg-white shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                <img 
                  src={
                ...
                <CardContent className="p-6">
                  <div className="flex items-center mb-2">
                    <Badge variant={getBadgeVariant(event.eventType)} className="mr-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button 
                      ...
                      className={`${getButtonColor(event.eventType)} transition-colors text-sm`}
                    >
                      {event.currentParticipants >= event.maxParticipants 
                        ? 'Full' 
                        : joinEventMutation.isPending 
            ...
            <div className="col-span-full text-center py-8 text-gray-500">
              No events currently scheduled. Check back soon for upcoming activities!
            </div>
          )}
        </div>
        ...
        <Card className="bg-white shadow-lg">
          <CardContent className="p-8">
            <h3 className="text-2xl font-bold text-text-dark text-center mb-8">University Societies & Partners</h3>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="text-center p-4 hover:bg-bg-light rounded-lg transition-colors">
                ...
                <h4 className="font-semibold text-text-dark mb-2">Environmental Society</h4>
                <p className="text-sm text-gray-600">Leading campus sustainability initiatives</p>
              </div>
              <div className="text-center p-4 hover:bg-bg-light rounded-lg transition-colors">
                <div className="bg-eco text-white w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Lightbulb className="h-8 w-8" />
                </div>
                <h4 className="font-semibold text-text-dark mb-2">Innovation Club</h4>
                <p className="text-sm text-gray-600">Developing eco-friendly solutions</p>
              </div>
              <div className="text-center p-4 hover:bg-bg-light rounded-lg transition-colors">
                <div className="bg-university text-white w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Heart className="h-8 w-8" />
                </div>
                <h4 className="font-semibold text-text-dark mb-2">Volunteer Corps</h4>
                <p className="text-sm text-gray-600">Community service and outreach</p>
              </div>
              <div className="text-center p-4 hover:bg-bg-light rounded-lg transition-colors">
                <div className="bg-warning text-text-dark w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Users className="h-8 w-8" />
                </div>
                <h4 className="font-semibold text-text-dark mb-2">Student Union</h4>
                <p className="text-sm text-gray-600">Coordinating campus-wide programs</p>
              </div>
            </div>
          </CardContent>
        </Card>
